...
func runBacktest() BacktestReport {
	...
	methodKeys := []string{"PAITO", "PAITO_LAMA", "SHIO", "MATH", "GABUNGAN"}
	methodLabels := map[string]string{
		"PAITO":      "Paito Baru (Multi-Lag+Delta)",
		"PAITO_LAMA": "Paito Lama (Freq+Overdue)",
		"SHIO":       "Shio",
		"MATH":       "Matrix",
		"GABUNGAN":   "Gabungan",
	}
	...
	for i := minHistory; i < len(all); i++ {
		...
		methodPreds := map[string][]string{
			...
			"PAITO_LAMA": predictPaitoLegacy(history),
